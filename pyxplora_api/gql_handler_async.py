...
class GQLHandler(HandlerGQL):
    ...
    async def subscribedCampaign_a(self, id, needDetail=False):
        return (await self.runAuthorizedGqlQuery_a(gq.CAMPAIGN_Q["subscribedQ"], {"uid": id, "needDetail": needDetail}))[
            "data"
        ]
    ...
    async def sendText_a(self, ownId, text):
        # ownUser id
        if (await self.runAuthorizedGqlQuery_a(gm.WATCH_M["sendChatTextM"], {"uid": ownId, "text": text}))["data"][
            "sendChatText"
        ] is not None:
            return True
        return False
    ...
    async def setReadChatMsg(self, ownId, msgId, id):
        return (await self.runAuthorizedGqlQuery_a(gm.WATCH_M["setReadChatMsgM"], {"uid": ownId, "msgId": msgId, "id": id}))[
            "data"
        ]
