...
class GQLHandler(HandlerGQL):
    ...
    async def login_a(self):
        data = (await self.runGqlQuery_a(gm.SIGN_M["issueTokenM"], self.variables))[
            "data"
        ]
        ...
        for contact in contacts["contacts"]["contacts"]:
            ...
            if self.userId == id:
                if contact["guardianType"] == "FIRST":
                    return (await self.runAuthorizedGqlQuery_a(query, variables))[
                        "data"
                    ][key]
    ...
    async def getContacts_a(self, ownId):
        ...
        return (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["contactsQ"], {"uid": ownId})
        )["data"]

    async def getWatchCount_a(self):
        return (
            await self.runAuthorizedGqlQuery_a(gq.QUERY["followRequestWatchCountQ"], {})
        )["data"]

    async def getWatches_a(self, ownId):
        return (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["watchesQ"], {"uid": ownId})
        )["data"]

    async def getWatchLastLocation_a(self, ownId):
        return (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["locateQ"], {"uid": ownId})
        )["data"]

    async def trackWatch_a(self, ownId):
        # tracking time - seconds
        res = (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["trackQ"], {"uid": ownId})
        )["data"]
    ...
    async def askWatchLocate_a(self, ownId):
        res = (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["askLocateQ"], {"uid": ownId})
        )["data"]
    ...
    async def getAlarms_a(self, ownId):
        return (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["alarmsQ"], {"uid": ownId})
        )["data"]

    async def getWifi_a(self, id):
        # without function?
        return (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["getWifisQ"], {"uid": id})
        )["data"]

    async def avatars_a(self, id):
        # without function?
        return (
            await self.runAuthorizedGqlQuery_a(gq.CAMPAIGN_Q["avatarsQ"], {"id": id})
        )["data"]
    ...
    async def chats_a(self, ownId):
        # ownUser id
        return (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["chatsQ"], {"uid": ownId})
        )["data"]
    ...
    async def campaignUserProfiles_a(self):
        return (
            await self.runAuthorizedGqlQuery_a(gq.MYINFO_Q["campaignUserProfilesQ"], {})
        )["data"]
    ...
    async def getReviewStatus_a(self, id):
        return (
            await self.runAuthorizedGqlQuery_a(gq.REVIEW_Q["getStatusQ"], {"uid": id})
        )["data"]

    async def countries_a(self):
        # Country Support
        return (await self.runAuthorizedGqlQuery_a(gq.UTILS_Q["countriesQ"], {}))[
            "data"
        ]

    async def safeZones_a(self, ownId):
        return (
            await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["safeZonesQ"], {"uid": ownId})
        )["data"]

    async def safeZoneGroups_a(self):
        return (await self.runAuthorizedGqlQuery_a(gq.WATCH_Q["safeZoneGroupsQ"], {}))[
            "data"
        ]
    ...
    async def silentTimes_a(self, ownId):
        return (
            await self.runAuthorizedGqlQuery_a(
                gq.WATCH_Q["silentTimesQ"], {"uid": ownId}
            )
        )["data"]
    ...
    async def addStep_a(self, stepCount):
        return (
            await self.runAuthorizedGqlQuery_a(
                gm.STEP_M["addM"], {"stepCount": stepCount}
            )
        )["data"]
    ...
    async def shutdown_a(self, ownId):
        ...
        return await self.isAdmin_a(
            ownId, gm.WATCH_M["shutdownM"], {"uid": ownId}, "reboot"
        )

    async def reboot_a(self, ownId):
        # ownUser id
        return await self.isAdmin_a(
            ownId, gm.WATCH_M["rebootM"], {"uid": ownId}, "reboot"
        )
    ...
    async def setEnableSlientTime_a(
